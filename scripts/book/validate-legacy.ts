...
export async function validateLegacy(outputDir: string): Promise<void> {
  ...
  for (const locale of locales) {
    ...
    const pdfPath = join(outputDir, data.legacyOutputs.pdf)
    const epubPath = join(outputDir, data.legacyOutputs.epub)

    // 1. Verify manifest paths resolve exactly
    if (!existsSync(pdfPath)) {
      throw new Error(`Legacy PDF missing at manifest path: ${pdfPath}`)
    }
    if (!existsSync(epubPath)) {
      throw new Error(`Legacy EPUB missing at manifest path: ${epubPath}`)
    }
  ...
  console.log('Legacy outputs successfully validated against manifest specifications!')
}
...
if (meta.main) {
  ...
  validateLegacy(targetDir).catch((err: unknown) => {
